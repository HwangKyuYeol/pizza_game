(()=>{"use strict";
const $=id=>document.getElementById(id),app=$("app"),cv=$("cv"),ctx=cv.getContext("2d"),hud=$("hud"),start=$("start"),count=$("count"),countText=$("countText"),pauseScreen=$("pauseScreen"),result=$("result"),hearts=$("hearts"),scoreEl=$("score"),timeEl=$("time"),bar=$("bar"),fever=$("fever");
...
let W,H,D=1,state="ready",score=0,lives=3,elapsed=0,left=30,last=0,spawn=0,combo=0,player,items=[],fx=[],parts=[],pointer=null,soundOn=true,audio=null,token="";
...
function reset(){score=0;lives=3;elapsed=0;left=30;spawn=0;combo=0;items=[];fx=[];parts=[];player.x=player.tx=W/2;token="";app.classList.remove("edge","shake");fever.classList.add("hidden");hudUpdate()}
...
function profile(){
if(elapsed<5)return{gap:rnd(.28,.36),bomb:.14,min:300,max:430,scale:1};
if(elapsed<10)return{gap:rnd(.23,.30),bomb:.18,min:350,max:490,scale:.96};
if(elapsed<15)return{gap:rnd(.19,.26),bomb:.24,min:410,max:560,scale:.92};
if(elapsed<20)return{gap:rnd(.16,.22),bomb:.30,min:470,max:650,scale:.88};
if(elapsed<25)return{gap:rnd(.14,.19),bomb:.36,min:540,max:740,scale:.84};
return{gap:rnd(.12,.17),bomb:.42,min:620,max:850,scale:.80}
}
function spawnItem(){
const p=profile(),bomb=Math.random()<p.bomb,type=bomb?"bomb":goods[Math.floor(Math.random()*goods.length)];
const base=(W<600?rnd(50,70):rnd(58,80))*p.scale;
const s=base*(bomb?.92:1);
items.push({
type,x:rnd(s*.6,W-s*.6),y:-s,s,
spd:rnd(p.min,p.max),r:rnd(-.3,.3),spin:rnd(-2.8,2.8),
wave:rnd(0,6.28),
ws:bomb?rnd(4.8,8.2):rnd(2.2,5.2),
amp:bomb?rnd(20,38):rnd(5,11)
});
spawn=p.gap
}
...
function collect(it,x){if(it.type==="bomb"){score=Math.max(0,score-10);lives--;combo=0;effect(x,player.y-10,"-10",true);burst(x,player.y,true);shake();if(navigator.vibrate)navigator.vibrate([80,35,100]);beep(90,.17,"sawtooth",.08);hudUpdate();if(lives<=0)finish(false,"life")}else{score+=10;combo++;effect(x,player.y-10,"+10");burst(x,player.y,false);beep(520+Math.min(combo,10)*20);if(combo===3)effect(W/2,H*.42,"GOOD!",false,1);if(combo===5)effect(W/2,H*.42,"GREAT!",false,1.05);if(combo===10)effect(W/2,H*.42,"PERFECT!!",false,1.15);if(score>=200)finish(true,"score")}}
function loop(now){if(state!=="playing")return;const dt=Math.min((now-last)/1000,.034);last=now;elapsed+=dt;left=30-elapsed;if(left<=0){left=0;hudUpdate();finish(score>=200,"time");return}const f=left<=10;fever.classList.toggle("hidden",!f);app.classList.toggle("edge",f);player.x+=(player.tx-player.x)*Math.min(1,dt*(W<700?22:14));spawn-=dt;if(spawn<=0)spawnItem();
for(let i=items.length-1;i>=0;i--){const it=items[i];it.y+=it.spd*dt;it.r+=it.spin*dt;it.wave+=it.ws*dt;const x=it.x+Math.sin(it.wave)*it.amp,hit=Math.abs(x-player.x)<player.w*.48+it.s*.28&&it.y+it.s*.4>=player.y&&it.y-it.s*.35<=player.y+player.h*.8;if(hit){collect(it,x);items.splice(i,1)}else if(it.y-it.s>H){if(it.type!=="bomb")combo=0;items.splice(i,1)}}
